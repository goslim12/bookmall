...
select date_format(max(hire_date),"%Y년%m월%d일")as "마지막신입사원입사일"
from employees;

-- 문제3.  //아몰랑~~~~~~~~~~~~~
-- 가장 오래 근속한 직원의 입사일은 언제인가요? 다음 형식으로 출력해주세요.

select emp_no,min(from_date),max(to_date),max(to_date)-min(from_date) as '근속년수'
from salaries
group by emp_no;
...
select max(e.birth_date)-min(e.birth_date) as '나이차이'
